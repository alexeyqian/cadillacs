...
def _push_enemies_apart(enemy, other, enemy_rect, other_rect):
    ...
    if smallest_x_push <= smallest_y_push:
        if enemy_rect.centerx <= other_rect.centerx:
            _split_push(enemy, other, -overlap_left, overlap_left)
        else:
            _split_push(enemy, other, overlap_right, -overlap_right)
    else:
        if enemy_rect.centery <= other_rect.centery:
            _split_push_y(enemy, other, -overlap_top, overlap_top)
        else:
            _split_push_y(enemy, other, overlap_bottom, -overlap_bottom)


def _split_push(enemy, other, enemy_push, other_push):
    enemy.x += enemy_push / 2
    other.x += other_push / 2


def _split_push_y(enemy, other, enemy_push, other_push):
