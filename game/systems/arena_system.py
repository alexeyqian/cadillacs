from game.settings import SCREEN_WIDTH

ENEMY_ARENA_ENTRY_MARGIN = 560

class ArenaSystem:
    @staticmethod
    def apply_bounds(game_state):
        level = game_state.level
        if not level.camera_locked or level.lock_x is None:
            return
        arena_left = level.lock_x
        arena_right = level.lock_x + SCREEN_WIDTH
        _clamp_entity(game_state.player, arena_left, arena_right)
        for enemy in game_state.enemies:
            if enemy.state == enemy.DEAD:
                continue
            _clamp_entity(
                enemy,
                arena_left - ENEMY_ARENA_ENTRY_MARGIN,
                arena_right + ENEMY_ARENA_ENTRY_MARGIN,
                use_collision_width=True,
            )


def _clamp_entity(entity, arena_left, arena_right, use_collision_width=False):
    width = entity.geometry.collision_box_w if use_collision_width else entity.width
    half_w = width // 2
    if entity.x - half_w < arena_left:
        entity.x = arena_left + half_w
    if entity.x + half_w > arena_right:
        entity.x = arena_right - half_w
