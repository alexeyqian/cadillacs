...
make combat timing data independent from sprite frames.
So the animation becomes the “skin”, while the attack definition becomes the “gameplay truth”.

The important idea:
Attack timing uses seconds / normalized time.
Animation uses frames.

ROM download:
https://archive.org/download/mame-0.221-roms-merged

Use a controller when something decides what one object should do.
A controller usually belongs to one entity.
Use a manager when something owns or coordinates many objects.
